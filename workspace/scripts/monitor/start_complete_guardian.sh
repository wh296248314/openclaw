...
# 创建完整的监控卫士主程序
cat > complete_guardian_main.py << 'PYTHON_EOF'
#!/usr/bin/env python3
"""
功能完整的增强版监控卫士主程序
"""
...
# 启动监控卫士
nohup python3 complete_guardian_main.py > "$LOG_FILE" 2>&1 &
COMPLETE_PID=$!
...
# 检查是否启动成功
if ps -p $COMPLETE_PID > /dev/null 2>&1; then
    echo $COMPLETE_PID > "$PID_FILE"
    echo "✅ 功能完整的增强版监控卫士启动成功 (PID: $COMP    echo "✅ 功能完整的增强版监控卫士启动成功 (PID: $COMPLETE_PID)"
    echo ""
    echo "📋 监控功能摘要:"
    echo "  • 四级优先级管理 (critical/high/medium/low)"
    echo "  • 系统资源监控 (CPU/内存/磁盘)"
    echo "  • 分级告警机制 (warning/error/critical)"
    echo "  • 任务进度跟踪和资源使用分析"
    echo "  • 实时仪表板显示"
    echo ""
    echo "📊 使用命令:"
    echo "  • 查看仪表板: python3 complete_guardian_main.py --dashboard"
    echo "  • 查看日志: tail -f $LOG_FILE"
    echo "  • 停止监控: kill $COMPLETE_PID"
    echo ""
    echo "🎯 立即可以监控的任务类型:"
    echo "  1. 🔴 Critical任务: 紧急修复、关键同步 (<30秒)"
    echo "  2. 🟠 High任务: 重要报告、API调用 (<60秒)"
    echo "  3. 🟡 Medium任务: 数据备份、常规处理 (<120秒)"
    echo "  4. 🟢 Low任务: 日志清理、后台分析 (<300秒)"
else
    echo "❌ 监控卫士启动失败"
    echo "请检查日志: $LOG_FILE"
    exit 1
